...
const skillGroups = [
  {
    title: "Game / XR",
    skills: [
      { name: "Unity / C#", level: 88, label: "상급" },
      { name: "GLSL / Shader", level: 72, label: "중상급" },
      { name: "XR Interaction Toolkit", level: 65, label: "중급" },
    ],
  },
  {
    title: "AI / ML",
    skills: [
      { name: "Python", level: 85, label: "상급" },
      { name: "TensorFlow / Keras", level: 78, label: "중상급" },
      { name: "XGBoost / SHAP / LIME", level: 80, label: "중상급" },
      { name: "scikit-learn", level: 70, label: "중급" },
    ],
  },
  {
    title: "Front-end",
    skills: [
      { name: "Figma / UI 설계", level: 70, label: "중상급" },
      { name: "Tailwind CSS", level: 65, label: "중급" },
      { name: "Storybook", level: 60, label: "중급" },
      { name: "Next.js", level: 40, label: "입문" },
    ],
  },
  {
    title: "Embedded",
    skills: [
      { name: "Raspberry Pi / Python", level: 60, label: "중급" },
      { name: "C", level: 55, label: "중급" },
    ],
  },
];
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "1rem" }}>
            {skillGroups.map((g, i) => (
              <motion.div key={g.title} custom={i} variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}
                ...
                <h3 style={{ fontSize: "13px", fontWeight: 600, color: "#1d9e75", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "1.25rem" }}>
                  {g.title}
                </h3>
                  ...
                  {g.skills.map((s) => (
                    <div key={s.name}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "5px" }}>
                        <span style={{ fontSize: "13px", color: "#aaa" }}>{s.name}</span>
                        <span style={{ fontSize: "11px", color: "#555" }}>{s.label}</span>
                      </div>
                      <div style={{ height: "4px", background: "#1a2a1a", borderRadius: "99px", overflow: "hidden" }}>
                        <div style={{ height: "100%", width: `${s.level}%`, background: "#1d9e75", borderRadius: "99px" }} />
                      </div>
                    </div>
                  ))}
                </div>
